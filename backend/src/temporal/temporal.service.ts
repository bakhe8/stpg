...
export class TemporalService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      this.logger.warn(
        'Temporal Server connection failed. Workflows will not be dispatched.',
      );
    }
  }
  ...
  async dispatchWorkflow(
    ...
    if (!this.client) {
      this.logger.warn(
        `Cannot dispatch workflow ${workflowName} because Temporal client is not connected.`,
      );
      return null;
    }
    ...
    try {
      ...
      this.logger.log(
        `Started workflow ${workflowName} with ID ${handle.workflowId}`,
      );
      return handle;
    } catch (err) {
